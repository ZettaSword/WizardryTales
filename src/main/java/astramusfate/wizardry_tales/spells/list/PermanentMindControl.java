...
public class PermanentMindControl extends SpellRay {
	...
	public static final String MAX_HEALTH = "min_health";

	public PermanentMindControl(){
		super(WizardryTales.MODID,"permanent_mind_control", SpellActions.POINT, false);
		addProperties(MAX_HEALTH);
	}
	...
	protected boolean onEntityHit(World world, Entity target, Vec3d hit, EntityLivingBase caster, Vec3d origin, int ticksInUse, SpellModifiers modifiers){
		...
		if(EntityUtils.isLiving(target)){
			...
			if(!canControl(target, caster, modifiers, getProperty(MAX_HEALTH).doubleValue())){
				if(!world.isRemote){
				...
				if(!world.isRemote){
					if(!PermanentMindControl.findMindControlTarget((EntityLiving)target, caster, world)){
	...
	public static boolean canControl(Entity target, EntityLivingBase caster, SpellModifiers modifiers, double max_health){
		...
		return target instanceof EntityLiving && target.isNonBoss() && !(target instanceof INpc)
				...
				.contains(EntityList.getKey(target.getClass())) && maxHealthCondition((EntityLiving) target,caster, max_health, modifiers );
	}
	...
	public static boolean maxHealthCondition(EntityLiving target, EntityLivingBase caster, double max_health, SpellModifiers modifiers){
		return (target.getMaxHealth() <= (max_health * modifiers.get(Sage.POTENCY))
	...
	private static void processTargeting(World world, EntityLiving entity, EntityLivingBase currentTarget){
		...
		if(entity.isPotionActive(WizardryPotions.mind_control) && PermanentMindControl.canControl(entity)){

			NBTTagCompound entityNBT = entity.getEntityData();

			if(entityNBT != null && entityNBT.hasUniqueId(PermanentMindControl.NBT_KEY)){

				Entity caster = EntityUtils.getEntityByUUID(world, entityNBT.getUniqueId(PermanentMindControl.NBT_KEY));
				...
				if(caster instanceof EntityLivingBase){
					...
					if(PermanentMindControl.findMindControlTarget(entity, (EntityLivingBase)caster, world)){
		...
		if(event.getEntity().ticksExisted % 50 == 0 && event.getEntityLiving().isPotionActive(WizardryPotions.mind_control)
			...
			if(((EntityLiving)event.getEntityLiving()).getAttackTarget() == null
				...
				if(entityNBT.hasUniqueId(MindControl.NBT_KEY)) {
					...
					Entity caster = EntityUtils.getEntityByUUID(entity.world, entityNBT.getUniqueId(PermanentMindControl.NBT_KEY));
					if (caster != null && !entity.world.isRemote){
						entity.getNavigator().tryMoveToEntityLiving(caster, 2.0F);
					}
				}
			}
		}
