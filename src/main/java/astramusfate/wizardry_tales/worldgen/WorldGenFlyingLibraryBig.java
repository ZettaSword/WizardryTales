...
public class WorldGenFlyingLibraryBig extends WorldGenSurfaceStructure {
    ...
    public WorldGenFlyingLibraryBig() {}

    @Override
    public String getStructureName() {
        return "flying_library_big";
    }

    @Override
    public ResourceLocation getStructureFile(Random random) {
        return new ResourceLocation(WizardryTales.MODID, "flying_library_big");
    }
    ...
    public boolean canGenerate(Random random, World world, int chunkX, int chunkZ){
        return ArrayUtils.contains(Tales.struct.flying_lib_big_dims, world.provider.getDimension())
                ...
                && Tales.struct.flying_lib_big > 0 && random.nextInt(Tales.struct.flying_lib_big) == 0;
    }
