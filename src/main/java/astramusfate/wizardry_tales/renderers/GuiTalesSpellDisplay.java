...
public class GuiTalesSpellDisplay {
    ...
    private static final Map<String, GuiSpellDisplay.Skin> skins = new LinkedHashMap<>(14); // 14 is the number of skins packaged with the mod
    ...
    public static Map<String, GuiSpellDisplay.Skin> getSkins(){
    ...
    @Nullable
    public static GuiSpellDisplay.Skin getSkin(String key){

        GuiSpellDisplay.Skin skin = skins.get(key);
    ...
    private static ItemStack getWandInUse(EntityPlayer player){
        if(ItemArtefact.isArtefactActive(player, TalesItems.casting_ring)
            ...
            return Thief.getItem(player, item -> item.getItem() instanceof ISpellCastingItem
                    && ((ISpellCastingItem) item.getItem()).getSpells(item).length >= 2);
        }
        return null;
    }
    ...
    private static void renderSpellHUD(EntityPlayer player, ItemStack wand, boolean mainHand, int width, int height, float partialTicks, boolean textLayer){
        ...
        GuiSpellDisplay.Skin skin = getSkin(Wizardry.settings.spellHUDSkin);
        ...
        try {
            ...
            for(IResource indexFile : indexFiles){
                ...
                for(Map.Entry<String, JsonElement> entry : json.entrySet()){
                    ...
                    skins.put(key, new GuiSpellDisplay.Skin(texture, metadata));
                }
            }
