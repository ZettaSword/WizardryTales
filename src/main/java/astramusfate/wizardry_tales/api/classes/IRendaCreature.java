 ...
 * {@link IRendaCreature#onLivingAttackEvent(LivingAttackEvent)
...
public interface IRendaCreature extends IEntityAdditionalSpawnData, IEntityOwnable {
	 ...
	 * {@link IRendaCreature#onDespawn()} for details.
	 */
	int getLifetime();
	 ...
	 * {@link IRendaCreature#getTargetSelector()}) and revenge targeting checks. This method is responsible for the
	...
	default boolean isValidTarget(Entity target){
		...
		if(AllyDesignationSystem.isValidTarget(this.getCaster(), target)){
			...
			if((target instanceof IMob || target instanceof IRendaCreature
		...
		if(player.isSneaking() && stack.getItem() instanceof ISpellCastingItem){
			...
			if(!player.world.isRemote && data != null && this.getCaster() == player){
				...
				if(data.selectedMinion != null && data.selectedMinion.get() == this){
					...
					// Selects this minion
					data.selectedMinion = new java.lang.ref.WeakReference<>((ISummonedCreature) this);
				}
				data.sync();
			}
			return true;
	...
	static void onLivingAttackEvent(LivingAttackEvent event){
		...
		if(event.getSource().getTrueSource() instanceof IRendaCreature){

			EntityLivingBase summoner = ((IRendaCreature)event.getSource().getTrueSource()).getCaster();
			...
			if(summoner != null){
				...
				if(DamageSafetyChecker.attackEntitySafely(event.getEntity(), newSource, event.getAmount(), event.getSource(), false)){
					...
					((IRendaCreature)event.getSource().getTrueSource()).onSuccessfulAttack(event.getEntityLiving());
